...
export default class AuthService {
  ...
  login () {
    if (this.customDomain) {
      ...
      window.location.href = `https://${this.cognitoDomain}.auth.${this.awsRegion}.amazoncognito.com/login?response_type=token&client_id=${this.poolClientId}&redirect_uri=${encodeURIComponent(window.location.href.split('#')[0])}`
    }
  }
  ...
  postAuth (token) {
    ...
    const AccountId = this.config.get('AWS_ACCOUNT_ID')
    const Logins = {}
    Logins[`cognito-idp.eu-west-1.amazonaws.com/${this.poolId}`] = token
    const req = {
      IdentityPoolId: this.idPoolId,
      AccountId,
      Logins
